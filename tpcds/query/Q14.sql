...
SELECT channel , i_category_id , sales , number_sales
FROM 
  (SELECT 0 AS key , 'store' channel , i_category_id , avg(ss_quantity*ss_list_price) sales, count(*) number_sales
   ...
   GROUP BY i_category_id) t1 ,
  (SELECT 0 AS key ,
          avg(ss_quantity*ss_list_price) average_sales
   FROM store_sales ,
        date_dim
   WHERE ss_sold_date_sk = d_date_sk
     AND d_year BETWEEN 2000 AND 2000 + 2) t2
WHERE t1.key = t2.key
  AND sales > average_sales
